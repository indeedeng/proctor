...
public class TestValueConverters {
    ...
    public void testBooleanConverter() throws ValueConversionException {
        final ValueConverter<Boolean> booleanValueConverter = ValueConverters.booleanValueConverter();
    ...
    private <E> void doCheckConversion(final ValueConverter<E> converter,
                                       final String rawValue,
                                       final E expected) throws ValueConversionException {
        final E actual = converter.convert(rawValue);
        assertEquals(converter.getClass().getSimpleName() + " should convert '" + rawValue + "' to " + expected, expected, actual);
    }

    private <E> void doCheckConversionWithExpectedException(final ValueConverter<E> converter,
                                                            final String rawValue) {
        try {
            converter.convert(rawValue);
            fail(converter.getClass().getSimpleName() + " should throw exception when converting " + rawValue);
