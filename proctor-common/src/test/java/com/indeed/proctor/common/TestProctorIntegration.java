...
public class TestProctorIntegration {
    ...
    private Proctor constructProctor() {
        ...
        final List<Allocation> abcAllocations =
                Collections.singletonList(
                        new Allocation(
                                null,
                                Arrays.asList(new Range[] {new Range(abcBucket0.getValue(), 1)})));

        final ConsumableTestDefinition abcTD =
                new ConsumableTestDefinition(
                        "1",
                        "${num > SOME_NUM && num < ANOTHER_NUM}",
                        TestType.ANONYMOUS_USER,
                        "abcsalt",
                        abcBuckets,
                        abcAllocations,
                        abcTestConstants,
                        "zingle boppity zip zop");
        ...
        final List<Allocation> defAllocations =
                Collections.singletonList(
                        new Allocation(
                                null,
                                Arrays.asList(
        ...
        final ConsumableTestDefinition defTD =
                new ConsumableTestDefinition(
                        "2",
                        "${proctor:contains(COUNTRIES, country) && T}",
                        TestType.ANONYMOUS_USER,
                        "defsalt",
                        defBuckets,
                        defAllocations,
        ...
        final List<TestBucket> ghiBuckets =
                Lists.newArrayList(ghiBucket0, ghiBucket1, ghiBucket2, ghiBucket3, ghiBucket4);
        ...
        final List<Allocation> ghiAllocations =
                Collections.singletonList(
                        new Allocation(
                                null,
                                Arrays.asList(
        ...
        final ConsumableTestDefinition ghiTD =
                new ConsumableTestDefinition(
                        "3",
                        "${proctor:contains(LANGUAGES, language)}",
                        TestType.ANONYMOUS_USER,
                        "ghisalt",
                        ghiBuckets,
                        ghiAllocations,
                        ghiTestConstants,
                        "jangle bing zimple plop");

        final ConsumableTestDefinition jklTD =
                        ...
                        TestDefinition.builder()
                                ...
                                .addAllocations(
                                        new Allocation(
                                                null,
                                                ImmutableList.of(
                                                        new Range(1, 0.5), new Range(2, 0.5))))
                                .build());
    ...
    private String calcGroups(
        ...
        final ProctorResult proctorResult =
                proctor.determineTestGroups(identifiers, context, forceGroups);
        final StringBuilder buckets = new StringBuilder();
        for (final Iterator<Entry<String, TestBucket>> iterator =
                        proctorResult.getBuckets().entrySet().iterator();
                iterator.hasNext(); ) {
            ...
            if (testBucket.getValue() >= 0) {
                ...
                buckets.append(testName).append(testBucket.getValue());
            }
        }
