...
public class ProctorPromoter {
    ...
    public ProctorPromoter(
        ...
        if (executor instanceof ThreadPoolExecutor) {
            ...
            exporter.export(
                    new ThreadPoolExecutorVarExports((ThreadPoolExecutor) executor),
                    "ProctorPromoter-pool-");
        }
    ...
    @Deprecated
    public void promoteTrunkToQa(
            final String testName,
            String trunkRevision,
            String qaRevision,
            String username,
            String password,
            String author,
            Map<String, String> metadata)
            throws StoreException, TestPromotionException {
        promote(
                testName,
                Environment.WORKING,
                trunkRevision,
    ...
    /** @deprecated Just use promote */
    @Deprecated
    public void promoteQaToProduction(
            final String testName,
            String qaRevision,
            String prodRevision,
            String username,
    ...
    /** @deprecated Just use promote */
    @Deprecated
    public void promoteTrunkToProduction(
            final String testName,
            String trunkRevision,
            String prodRevision,
            String username,
    ...
    public void promote(
            final String testName,
            final Environment srcBranch,
            final String srcRevision,
            final Environment destBranch,
            final String destRevision,
            final String username,
            final String password,
            final String author,
            final Map<String, String> metadata)
            throws TestPromotionException, StoreException {
        LOGGER.info(
                String.format(
                        "%s : Promoting %s from %s r%s to %s r%s",
                        username, testName, srcBranch, srcRevision, destBranch, destRevision));
        ...
        final String version =
                TestDefinitionUtil.getResolvedLastVersion(dest, testName, destBranch).getVersion();
        ...
        if (knownDestRevision.length() == 0 && destRevision.length() > 0) {
            throw new TestPromotionException(
                    "Positive revision r"
                            + destRevision
        ...
        if (!knownDestRevision.equals(UNKNOWN_REVISION)
                && knownDestRevision.length() > 0
                && destRevision.length() == 0) {
            throw new TestPromotionException(
                    "Non-Positive revision r"
                            + destRevision
                            + " given for destination ( "
                            + destBranch
                            + " ) but '"
        ...
        if (srcHistory.isEmpty()) {
            throw new TestPromotionException(
                    "Could not find history for " + testName + " at revision " + srcRevision);
        }
        final Revision srcVersion = srcHistory.get(0);

        // Update the Test Definition Version to the svn-revision of the source (if it is a migrated
        // commit)
        final String effectiveRevision =
                GitProctorUtils.resolveSvnMigratedRevision(
                        srcVersion, Environment.WORKING.getName());

        if (isSrcTrunk) {
            // If source is trunk, we want to set the version of the test-matrix to be the revision
            // on trunk
        ...
        if (!knownDestRevision.equals(UNKNOWN_REVISION) && knownDestRevision.length() > 0) {
            ...
            if (dest.getCurrentTestDefinition(testName) == null) {
                ...
                throw new IllegalArgumentException(
                        "Test '"
                                + testName
                                + "' has been deleted in destination, not allowed to promote again.");
            }
            ...
            final String commitMessage =
                    formatCommitMessage(
                            ...
                            effectiveRevision,
                            destBranch,
                            srcVersion.getMessage());
            LOGGER.info(
                    String.format(
                            "%s : Committing %s from %s r%s to %s r%s",
                            username, testName, srcBranch, srcRevision, destBranch, destRevision));
            dest.updateTestDefinition(
                    username, password, author, destRevision, testName, d, metadata, commitMessage);
        } else {
            final String commitMessage =
                    formatCommitMessage(
                            testName,
                            ...
                            destBranch,
                            srcVersion.getMessage());
            dest.addTestDefinition(
                    username, password, author, testName, d, metadata, commitMessage);
        }
    }
    ...
    private static String formatCommitMessage(
            final String testName,
            final Environment src,
            final String srcRevision,
            final Environment dest,
            final String comment) {
        final StringBuilder sb = new StringBuilder();
        sb.append(
                String.format(
                        "Promoting %s (%s r%s) to %s",
                        testName, src.getName(), srcRevision, dest.getName()));
    ...
    /**
     * Retrieve latest revision and version in each environment from ProctorStore and then return
     * them as EnvironmentVersion. Prefer TestDefinitionUtil.getResolvedLastVersion when possible
     */
    @CheckForNull
    public EnvironmentVersion getEnvironmentVersion(final String testName) {
        ...
        final Future<SingleEnvironmentVersion> trunkFuture =
                executor.submit(
                        () ->
                                TestDefinitionUtil.getResolvedLastVersion(
                                        trunk, testName, Environment.WORKING));
        final Future<SingleEnvironmentVersion> qaFuture =
                executor.submit(
                        () ->
                                TestDefinitionUtil.getResolvedLastVersion(
                                        qa, testName, Environment.QA));
        final Future<SingleEnvironmentVersion> productionFuture =
                executor.submit(
                        () ->
                                TestDefinitionUtil.getResolvedLastVersion(
                                        production, testName, Environment.PRODUCTION));
        try {
            ...
            LOGGER.error(
                    "Timed out when retrieving latest version for trunk or qa or production", e);
    ...
    private static List<Revision> getHistoryFromRevision(
            final ProctorStore src, final String testName, final String srcRevision)
            throws StoreException {
        return src.getHistory(testName, srcRevision, 0, 1);
    }

    // @Nullable
    private static TestDefinition getTestDefinition(
            final ProctorStore store, final String testName, String version) throws StoreException {
