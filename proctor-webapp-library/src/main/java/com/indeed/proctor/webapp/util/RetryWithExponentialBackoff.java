...
public class RetryWithExponentialBackoff {
    ...
    /**
     * This tries supplier.get() up to maxAttemptCount times. Between each attempts, it has interval
     * time which is increased by twice. The initial interval time is 1 second. The maximum interval
     * time is 2^maxAttemptIntervalIncrease seconds.
     *
     ...
     * @return the result of supplier.get() wrapped by Optional. It can be empty if it failed after
        ...
        for (int attemptCount = 0; attemptCount < maxAttemptCount; attemptCount++) {
            if (attemptCount > 0) {
                final long sleepTimeMillis =
                        (1L << Math.min(attemptCount - 1, maxAttemptIntervalIncrease)) * 1000;
