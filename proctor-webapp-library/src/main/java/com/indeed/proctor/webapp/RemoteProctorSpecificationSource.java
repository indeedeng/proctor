...
public class RemoteProctorSpecificationSource extends DataLoadingTimerTask
        implements ProctorSpecificationSource {
    private static final Logger LOGGER =
            LogManager.getLogger(RemoteProctorSpecificationSource.class);
    ...
    private final Map<Environment, ImmutableMap<AppVersion, RemoteSpecificationResult>>
            applicationMapByEnvironment = Maps.newConcurrentMap();

    private final Map<Environment, ProctorReader> proctorReaderMap;

    private final HttpClient httpClient;

    public RemoteProctorSpecificationSource(
    ...
    @Override
    public RemoteSpecificationResult getRemoteResult(
            final Environment environment, final AppVersion version) {
        return Optional.ofNullable(applicationMapByEnvironment.get(environment))
                .map(applicationMap -> applicationMap.get(version))
                .orElseGet(
                        () -> RemoteSpecificationResult.failures(version, Collections.emptyMap()));
    }

    @Override
    public Map<AppVersion, RemoteSpecificationResult> loadAllSpecifications(
    ...
    @Override
    public Map<AppVersion, ProctorSpecifications> loadAllSuccessfulSpecifications(
            final Environment environment) {
        final ImmutableMap<AppVersion, RemoteSpecificationResult> applicationsMap =
                applicationMapByEnvironment.get(environment);
        if (applicationsMap == null) {
        ...
        return applicationsMap.entrySet().stream()
                ...
                .collect(
                        Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getSpecifications()));
    }
    ...
    public Set<AppVersion> activeClients(final Environment environment, final String testName) {
        ...
        final ConsumableTestDefinition testDefinition =
                getCurrentConsumableTestDefinition(environment, testName);
    ...
    private boolean refreshInternalCache(final Environment environment) {
        LOGGER.info(
                "Refreshing internal list of ProctorSpecifications for environment " + environment);
        ...
        final ImmutableMap.Builder<AppVersion, RemoteSpecificationResult> allResults =
        ...
        for (final ProctorClientApplication client : clients) {
            final AppVersion appVersion =
                    new AppVersion(client.getApplication(), client.getVersion());
        ...
        for (final AppVersion appVersion : apps.keySet()) {
            ...
            futures.put(
                    appVersion,
                    httpExecutor.submit(() -> internalGet(appVersion, callableClients)));
        }
        futures.forEach(
                (version, future) -> {
                    try {
        ...
        // TODO (parker) 9/6/12 - Fail if we do not have 1 specification for each
        // <Application>.<Version>
        // should we update the cache?
        if (!appVersionsToCheck.isEmpty()) {
            LOGGER.warn(
                    "Failed to load any specification for the following AppVersions: "
    ...
    /**
     * Fetch a specification from a list of instances (for a single application and version) It
     * iterates instances until it success to read specification or find it doesn't expose endpoint
     * for spec.
     */
    private RemoteSpecificationResult internalGet(
            final AppVersion version, final List<ProctorClientApplication> clients) {
        ...
        if (!failures.isEmpty()) {
            LOGGER.info(
                    "Failed to get specifications from " + version,
    ...
    /**
     * try visiting private/v or private/proctor/specification to get specification The
     * specifications are used by proctor webapp to validate test edit/promotion or to detect usage
     * of test definitions.
     *
     * <p>Applications are expected to expose their specifications via either of these endpoints
     *
     * <p>* /private/proctor/specification by
     * com.indeed.proctor.consumer.ViewProctorSpecificationServlet
     *
     * <p>* /private/v?ns=JsonProctorLoaderFactory by
     * com.indeed.util.varexport.servlet.ViewExportedVariablesServlet The variable is exposed in
     * com.indeed.proctor.common.JsonProctorLoaderFactory
     */
    private ProctorSpecifications fetchSpecification(final ProctorClientApplication client)
            throws IOException {
        // This URL is where we expose variables by ViewExportedVariablesServlet
        final String varExportUrl =
                client.getBaseApplicationUrl() + "/private/v?ns=JsonProctorLoaderFactory";

        final ProctorSpecifications specFromVarExport =
                parseExportedVariables(fetchContentFromUrl(varExportUrl), client);
        ...
        final String viewSpecUrl =
                client.getBaseApplicationUrl() + "/private/proctor/specification";
        try {
            return parseSpecificationFromServlet(fetchContentFromUrl(viewSpecUrl), client);
        } catch (final IOException e) {
    ...
    private String fetchContentFromUrl(final String urlString) throws IOException {
    ...
    private ConsumableTestDefinition getCurrentConsumableTestDefinition(
            final Environment environment, final String testName) {
        try {
            final TestDefinition testDefinition =
                    proctorReaderMap.get(environment).getCurrentTestDefinition(testName);
            if (testDefinition != null) {
                return ProctorUtils.convertToConsumableTestDefinition(testDefinition);
            }
        } catch (final StoreException e) {
            LOGGER.warn(
                    "Failed to get current consumable test definition for "
                            + testName
                            + " in "
                            + environment,
                    e);
    ...
    /**
     * Parse generated string from com.indeed.util.varexport.servlet.ViewExportedVariablesServlet
     * for variables in the namespace of JsonProctorLoaderFactory
     *
     * <p>JsonProctorLoaderFactory is expected to expose specification variable with the variable
     * name - specification (earlier than 1.7.5) or - specification-file_name.json (1.7.5 or later)
     *
     * <p>Example string is
     *
     * <p>specification-spec.json={"providedContext"\:{"country"\:"String"}},"tests"\:{},"dynamicFilters"\:[]}
     * exporter-start-time=2019-09-18T02\:28\:24CDT
     */
    @VisibleForTesting
    static ProctorSpecifications parseExportedVariables(
            final String content, final ProctorClientApplication client) throws IOException {
        final Properties exportedVariables = VarExportedVariablesDeserializer.deserialize(content);
        ...
        for (final String key : exportedVariables.stringPropertyNames()) {
            ...
            try {
                ...
                LOGGER.error(
                        "Failed to parse variable " + key + ": " + json + " from " + client, e);
            }
        }
        if (specifications.isEmpty()) {
            throw new IOException(
                    "no specification can be parsed from exported variables from " + client);
        }
        return new ProctorSpecifications(specifications);
    }

    private static ProctorSpecifications parseSpecificationFromServlet(
            final String jsonString, final ProctorClientApplication client) throws IOException {
        final com.indeed.proctor.common.SpecificationResult specificationResult =
                OBJECT_MAPPER.readValue(
                        jsonString, com.indeed.proctor.common.SpecificationResult.class);
        ...
        return new ProctorSpecifications(
                Collections.singleton(specificationResult.getSpecification()));
    }
}
