...
class AutoPromoter {
    private static final Logger LOGGER = LogManager.getLogger(AutoPromoter.class);
    ...
    public void maybeAutoPromote(
        ...
        try {
            switch (autopromoteTarget) {
                case QA:
                    final String currentRevision =
                            getCurrentVersion(testName, trunkStore).getRevision();
            ...
            throw new AutoPromoteFailedException(
                    "Test Creation/Edit succeeded. However, the test was not promoted automatically to QA/Production.",
                    exception);
        }
    }
        ...
        if (existingTestDefinition == null) {
            if (!isAllInactiveTest(testDefinitionToUpdate)) {
                throw new IllegalArgumentException(
                        "auto-promote is prevented because there is no existing test definition and the test is not 100% inactive.");
            }
            ...
            final String currentRevision =
                    getCurrentVersionIfDirectlyFollowing(testName, previousRevision, trunkStore)
                            .getRevision();
    ...
    /**
     * Promote a test to QA and Prod without checking changes This promotion logic is only for test
     * creation
     */
    @VisibleForTesting
    void doPromoteNewlyCreatedInactiveTestToQaAndProd(
        ...
        editAndPromoteJob.doPromoteInternal(
                testName,
                username,
                password,
                author,
    ...
    /** Promote a test to QA and Prod If it's not allocation-only change, it doesn't promote */
    @VisibleForTesting
    void doPromoteExistingTestToQaAndProd(
        ...
        if (!isAllocationOnlyChange(existingTestDefinition, testDefinitionToUpdate)) {
            throw new IllegalArgumentException(
                    "auto-promote is prevented because it isn't an allocation-only change.");
        }

        job.log(
                "allocation only change, checking against other branches for auto-promote capability for test "
                        + testName
                        + "\nat QA revision "
                        + qaRevision
                        + " and PRODUCTION revision "
                        + prodRevision);
     ...
     * @param testDefinitionToUpdate for checking allocation only change condition. This can be null
     *     iff verifyAllocationOnly is false.
     * @param requestParameterMap
     * @param job the edit job which runs this edit process
     * @param currentRevision the revision of the source environment
     * @param targetRevision the target revision to get the target test definition to check
     *     allocation only change condition. This can be EnvironmentVersion.UNKNOWN_REVISION iff
     *     verifyAllocationOnly is false.
    ...
    void doPromoteTestToEnvironment(
        ...
        if (verifyAllocationOnlyChange
                && targetRevision.equals(EnvironmentVersion.UNKNOWN_REVISION)) {
        ...
        if (!QA.equals(targetEnv) && !PRODUCTION.equals(targetEnv)) {
            throw new IllegalArgumentException(
                    "Promotion target environment " + targetEnv.getName() + " is invalid.");
        }

        final TestDefinition targetTestDefinition =
        ...
        if (verifyAllocationOnlyChange) {
            if (testDefinitionToUpdate == null) {
                ...
                LOGGER.error(
                        "Failed to verify if the test change is allocation only change due to lack of test definition to update.");
                LOGGER.error(
                        String.format("testName: %s,targetEnv: %s", testName, targetEnv.getName()));
                throw new IllegalStateException(
                        "Bug: Failed to verify if the test change is allocation only change due to lack of test definition to update.");
            }
            if (!isAllocationOnlyChange(targetTestDefinition, testDefinitionToUpdate)) {
                throw new IllegalArgumentException(
                        "Not auto-promote to "
                                + targetEnv.getName()
                                + " because it isn't an allocation-only change.");
            }
        }
    ...
    static boolean isAllInactiveTest(final TestDefinition testDefinition) {
        ...
        final Map<Integer, TestBucket> valueToBucket =
                testDefinition.getBuckets().stream()
    ...
    private static boolean isInactiveBucket(final TestBucket bucket) {
        ...
        return bucket.getValue() == -1
                && ("inactive".equalsIgnoreCase(bucket.getName())
                        || "disabled".equalsIgnoreCase(bucket.getName()));
    }

    /**
     * Get current revision of the test which has been updated in the edit job. This method assumes
     * that the test is updated exactly once after {@code previousRevision}
     *
     ...
     * @return Gets the current revision of {@code testName} to autopromote. {@code
     *     previousRevision} is used to check for any modification since this edit process began.
     * @throws IllegalStateException the number of history is less than 2 or {@code
     *     previousRevision} is not the same as the revision of the second latest history.
     */
    @Nonnull
    private static Revision getCurrentVersionIfDirectlyFollowing(
            final String testName, final String previousRevision, final ProctorStore store) {
        final List<Revision> histories = TestDefinitionUtil.getTestHistory(store, testName, 2);

        if (histories.size() < 2) {
            throw new IllegalStateException(
                    "Test history should have at least 2 versions for edit and promote. "
                            + "Actually only has "
                            + histories.size()
                            + " versions");
        }
        if (!histories.get(1).getRevision().equals(previousRevision)) {
            throw new IllegalStateException(
                    "The passed previous revision was "
                            + previousRevision
                            + " but the previous revision from the history is "
                            + histories.get(1)
    ...
    /**
     * Get current revision of the test. This method is used after saving the new test.
     *
     * @param testName the name of the test
     * @param store the ProctorStore for the environment
    ...
    private static Revision getCurrentVersion(final String testName, final ProctorStore store) {
        ...
        if (histories.size() == 0) {
            throw new IllegalStateException(
                    "Test hasn't been created. Failed to find the version for autopromote.");
        }
        return histories.get(0);
    }

    static boolean isAllocationOnlyChange(
            final TestDefinition existingTestDefinition,
            final TestDefinition testDefinitionToUpdate) {
        ...
        if (nullRule && testDefinitionToUpdate.getRule() != null) {
            return false;
                ...
                && !existingTestDefinition.getRule().equals(testDefinitionToUpdate.getRule())) {
            return false;
        }
        if (!existingTestDefinition.getConstants().equals(testDefinitionToUpdate.getConstants())
                || !existingTestDefinition
                        .getSpecialConstants()
                        .equals(testDefinitionToUpdate.getSpecialConstants())
                || !existingTestDefinition
                        .getTestType()
                        .equals(testDefinitionToUpdate.getTestType())
        ...
        for (int i = 0; i < existingAllocations.size(); i++) {
            ...
            if (existingAllocations.get(i).getRule() == null
                    && allocationsToUpdate.get(i).getRule() != null) {
                return false;
            } else if (existingAllocations.get(i).getRule() != null
                    && !existingAllocations
                            .get(i)
                            .getRule()
                            .equals(allocationsToUpdate.get(i).getRule())) {
                return false;
            }
    ...
    private static boolean isNewBucketAdded(
        ...
        final Map<Integer, Double> existingAllocRangeMap =
                generateAllocationRangeMap(existingAllocationRanges);
        final Map<Integer, Double> allocToUpdateRangeMap =
                generateAllocationRangeMap(allocationToUpdateRanges);
    ...
    static class AutoPromoteFailedException
            extends EditAndPromoteJob.PostEditActionFailedException {
