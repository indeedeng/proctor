...
/**
 * @author parker This class is here to get around a circular dependency in the
 *     applicationContext.xml:
 *     <p>Specifically, the SvnProctorStoreFactory requires a ScheduledExecutorService to intialize
 *     its SvnProctorStores.
 *     <p>And we'd like to schedule the ProctorPromoter (but that needs the ProctorStore to be
 *     initialized).
 */
@Component
public class ScheduledTasks implements InitializingBean {
    ...
    // No longer used - only included to force instantiation of ProctorStores required for
    // ProctorDependencyManager
    ...
    public void afterPropertiesSet() throws Exception {
        scheduledExecutorService.scheduleWithFixedDelay(
                proctorSpecificationSource, 1, 10, TimeUnit.MINUTES);

        if (scheduledExecutorService instanceof ThreadPoolExecutor) {
            VarExporter.forNamespace(getClass().getSimpleName())
                    .export(
                            new ThreadPoolExecutorVarExports(
