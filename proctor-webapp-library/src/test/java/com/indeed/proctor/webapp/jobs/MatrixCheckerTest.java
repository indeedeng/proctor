...
public class MatrixCheckerTest {
    ...
    public void testCheckMatrix() {
        ...
        final ProctorSpecification targetSpec =
                stubSpecification(targetTestName, stubTestSpecification(expectedBucketValue));
        final ProctorSpecification unrelatedSpec =
                stubSpecification(unrelatedTestName, stubTestSpecification(expectedBucketValue));

        final ProctorSpecificationSource source = mock(ProctorSpecificationSource.class);
        ...
        final RemoteSpecificationResult result =
                stubResult(
                        ...
                        unrelatedSpec,
                        unrelatedSpec);
        when(source.getRemoteResult(eq(environment), eq(app))).thenReturn(result);
        ...
        assertThat(checker.checkMatrix(environment, targetTestName, null).getErrors())
                .containsExactly("app@v1 requires test 'sample_tst'");
    }

    @Test
    public void testGetErrorMessage_loadError() {
        final AppVersion appVersion =
                new AppVersion("TestApp", "a5efcc49c813e56aa7e35333aa8590515f5c061e");
        final ProctorLoadResult result =
                new ProctorLoadResult(
                        ImmutableMap.of(
                                "check_tst",
    ...
    public void testGetErrorMessage_missingTest() {
        final AppVersion appVersion =
                new AppVersion("TestApp", "a5efcc49c813e56aa7e35333aa8590515f5c061e");
        final ProctorLoadResult result =
                new ProctorLoadResult(Collections.emptyMap(), ImmutableSet.of("check_tst"), true);
        assertEquals(
    ...
    public void testGetErrorMessage_success() {
        final AppVersion appVersion =
                new AppVersion("TestApp", "a5efcc49c813e56aa7e35333aa8590515f5c061e");
        final ProctorLoadResult result =
                new ProctorLoadResult(Collections.emptyMap(), Collections.emptySet(), true);
        assertEquals("", MatrixChecker.getErrorMessage(appVersion, result));
    }
    ...
    private static RemoteSpecificationResult stubResult(
        ...
        when(result.getSpecifications())
                .thenReturn(new ProctorSpecifications(Arrays.asList(specifications)));
        return result;
    }

    private static ProctorSpecification stubSpecification(
            final String testName, final TestSpecification testSpecification) {
    ...
    private static TestSpecification stubTestSpecification(final int... bucketValues) {
    ...
    private static TestDefinition stubDefinition(final int bucketValue) {
        ...
        final List<TestBucket> buckets =
                ImmutableList.of(new TestBucket("grp" + bucketValue, bucketValue, "grp"));
        final List<Range> ranges = ImmutableList.of(new Range(bucketValue, 1.0));
