...
public class ProctorPromoterTest {
    @Mock private ProctorStore trunk;
    @Mock private ProctorStore qa;
    @Mock private ProctorStore production;
    ...
    private static final List<Revision> PRODUCTION_HISTORY =
            Collections.singletonList(new Revision(PROD_REVISION, null, null, COMMIT_MESSAGE));

    @Before
    public void setup() throws StoreException {
        Mockito.when(trunk.getTestDefinition(TEST_NAME, TRUNK_REVISION))
                .thenReturn(stubTestDefinition(TRUNK_REVISION));
    ...
    public void promoteTrunkToQA() throws StoreException, ProctorPromoter.TestPromotionException {
        ...
        proctorPromoter.promoteTrunkToQa(
                TEST_NAME, TRUNK_REVISION, QA_REVISION, USERNAME, PASSWORD, AUTHOR, METADATA);
        Mockito.verify(proctorPromoter, Mockito.times(1))
                .promote(
                        TEST_NAME,
                        Environment.WORKING,
                        TRUNK_REVISION,
    ...
    public void promoteQAToProduction()
            throws StoreException, ProctorPromoter.TestPromotionException {
        Mockito.doNothing()
                .when(proctorPromoter)
                .promote(
                        Mockito.eq(TEST_NAME),
        ...
        proctorPromoter.promoteQaToProduction(
                TEST_NAME, QA_REVISION, PROD_REVISION, USERNAME, PASSWORD, AUTHOR, METADATA);
        Mockito.verify(proctorPromoter, Mockito.times(1))
                .promote(
                        TEST_NAME,
                        Environment.QA,
    ...
    public void promoteTrunkToProduction()
        ...
        proctorPromoter.promoteTrunkToProduction(
                TEST_NAME, TRUNK_REVISION, PROD_REVISION, USERNAME, PASSWORD, AUTHOR, METADATA);
        Mockito.verify(proctorPromoter, Mockito.times(1))
                .promote(
                        TEST_NAME,
                        Environment.WORKING,
    ...
    @Test
    public void promoteWhenRevDoesNotExistInDestFails()
            throws StoreException, ProctorPromoter.TestPromotionException {
        Mockito.when(qa.getCurrentTestDefinition(TEST_NAME))
                .thenReturn(stubTestDefinition(EMPTY_QA_REVISION)); // can never happen?

        assertThatThrownBy(
                        () ->
                                proctorPromoter.promote(
                                        TEST_NAME,
                                        Environment.WORKING,
                                        TRUNK_REVISION,
                                        Environment.QA,
    ...
    @Test
    public void promoteWhenRevIsSpecifiedAsEmptyButExistsInDestFails()
            throws StoreException, ProctorPromoter.TestPromotionException {
        Mockito.when(qa.getHistory(TEST_NAME, 0, 1)).thenReturn(QA_HISTORY);
        Mockito.when(qa.getCurrentTestDefinition(TEST_NAME))
                .thenReturn(stubTestDefinition(QA_REVISION));
    ...
    @Test
    public void promoteWhenSrcHistoryIsEmptyFails()
            throws StoreException, ProctorPromoter.TestPromotionException {
        Mockito.when(qa.getHistory(TEST_NAME, 0, 1)).thenReturn(QA_HISTORY);
        Mockito.when(qa.getCurrentTestDefinition(TEST_NAME))
                .thenReturn(stubTestDefinition(QA_REVISION));
    ...
    @Test
    public void promoteWhenRevIsNewAddsTestDefinition()
            throws StoreException, ProctorPromoter.TestPromotionException {
        ...
        Mockito.verify(qa)
                .addTestDefinition(
                        ...
                        Mockito.eq(AUTHOR),
                        Mockito.eq(TEST_NAME),
                        Mockito.any(TestDefinition.class),
                        Mockito.eq(METADATA),
                        Mockito.eq("Promoting tn (trunk rtr) to qa\n" + "\n" + COMMIT_MESSAGE));
    }

    @Test
    public void promoteWhenHistoryDoesNotExistFails()
            throws StoreException, ProctorPromoter.TestPromotionException {
        Mockito.when(trunk.getHistory(TEST_NAME, TRUNK_REVISION, 0, 1)).thenReturn(TRUNK_HISTORY);
    ...
    @Test
    public void promoteWhenHistoryDoesNotHaveLatestDestRev()
            throws StoreException, ProctorPromoter.TestPromotionException {
    ...
    @Test
    public void promoteAgainWhenTestInDestinationHasBeenDeleted()
            throws StoreException, ProctorPromoter.TestPromotionException {
        Mockito.when(trunk.getHistory(TEST_NAME, TRUNK_REVISION, 0, 1)).thenReturn(TRUNK_HISTORY);

        Mockito.when(qa.getHistory(TEST_NAME, 0, 1)).thenReturn(QA_HISTORY);
        Mockito.when(qa.getCurrentTestDefinition(TEST_NAME))
                .thenReturn(stubTestDefinition(QA_REVISION), null); // not sure how this makes sense
    ...
    @Test
    public void promoteWhenRevExistsUpdatesTestDefinition()
            throws StoreException, ProctorPromoter.TestPromotionException {
        Mockito.when(trunk.getHistory(TEST_NAME, TRUNK_REVISION, 0, 1)).thenReturn(TRUNK_HISTORY);
        Mockito.when(qa.getHistory(TEST_NAME, 0, 1)).thenReturn(QA_HISTORY);
        Mockito.when(qa.getCurrentTestDefinition(TEST_NAME))
    ...
    public void getEnvironmentVersion()
            throws StoreException, InterruptedException, ExecutionException, TimeoutException {
        ...
        Mockito.when(executorService.submit(Mockito.any(Callable.class))).thenReturn(mockedFuture);

        final EnvironmentVersion environmentVersion =
                proctorPromoter.getEnvironmentVersion(TEST_NAME);
        ...
        Assertions.assertThat(environmentVersion.getProduction())
                .isEqualTo(PRODUCTION_HISTORY.get(0));
