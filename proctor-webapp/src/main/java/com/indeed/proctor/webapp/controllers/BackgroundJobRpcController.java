 ...
 *
 * <p>Endpoints: - GET /rpc/jobs/list - GET /rpc/jobs/status?id= - GET /rpc/jobs/cancel?id= - GET
 * /rpc/jobs/test
 */
@Controller
...
public class BackgroundJobRpcController {
    ...
    @Autowired
    public BackgroundJobRpcController(
            final BackgroundJobManager manager,
    ...
    @ResponseBody
    public JsonResponse<BackgroundJob.JobInfo> doGetJobStatus(
            @RequestParam("id") final UUID jobId) {
    ...
    public View doCancelJob(@RequestParam("id") final UUID jobId) {
        ...
        if (job == null) {
            ...
            final JsonResponse<BackgroundJobResponseModel> response =
                    new JsonResponse<>(result, true, null);
                            ...
                            public Boolean execute(final BackgroundJob<Boolean> job)
                                ...
                                while (true) {
                                    ...
                                    if (sleepms > 0) {
                                        ...
                                        job.log(
                                                String.format(
                                                        "Elapsed = %.3f seconds, sleeping for %s ms",
                                                        elapsedSec, sleepms));
