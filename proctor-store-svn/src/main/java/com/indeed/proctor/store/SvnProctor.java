...
public class SvnProctor extends FileBasedProctorStore {
    ...
    */
    ...
    public SvnProctor(final String svnPath, final String username, final String password)
            throws IOException {
    ...
    public List<Revision> getHistory(final String test, final int start, final int limit)
        ...
        return getSvnCore()
                .doWithClientAndRepository(
                        new SvnPersisterCore.SvnOperation<List<Revision>>() {
                            @Override
                            public List<Revision> execute(
                                ...
                                return getHistory(
                                        test, String.valueOf(latestRevision), start, limit);
                            }

                            @Override
                            public StoreException handleException(final Exception e)
                                    throws StoreException {
                                throw new StoreException.ReadException(
                                        "Unable to get older revisions for " + test, e);
                            }
                        });
    }
    ...
    public List<Revision> getHistory(
            final String test, final String version, final int start, final int limit)
            throws StoreException {
                ...
                .doWithClientAndRepository(
                        new SvnPersisterCore.SvnOperation<List<Revision>>() {
                            ...
                            public List<Revision> execute(
                                ...
                                return getSVNLogs(
                                        clientManager, targetPaths, svnRevision, start, limit);
                            }
                            ...
                            public StoreException handleException(final Exception e)
                                    throws StoreException {
                                throw new StoreException.ReadException(
                                        "Unable to get older revisions for "
                                                + test
                                                + " r"
                                                + revision,
                                        e);
    ...
    public Map<String, List<Revision>> getAllHistories() throws StoreException {
        final TestMatrixDefinition testMatrixDefinition =
                getCurrentTestMatrix().getTestMatrixDefinition();
    ...
    public String getLatestVersion() throws StoreException {
        return getSvnCore()
                .doWithClientAndRepository(
                        new SvnPersisterCore.SvnOperation<String>() {
                            @Override
                            public String execute(
                                ...
                                final FilterableSVNLogEntryHandler handler =
                                        new FilterableSVNLogEntryHandler();

                                // In order to get history is "descending" order, the startRevision
                                // should be the one closer to HEAD
                                logClient.doLog(
                                        svnUrl,
                                        targetPaths, /* pegRevision */
                                        SVNRevision.HEAD,
                                        svnRevision,
                                        SVNRevision.create(1),
                                        /* stopOnCopy */ false, /* discoverChangedPaths */
                                        false, /* includeMergedRevisions */
                                        false,
                                        /* limit */ 1,
                                        new String[] {SVNRevisionProperty.LOG},
                                        handler);
                                final SVNLogEntry entry =
                                        handler.getLogEntries().size() > 0
                                                ? handler.getLogEntries().get(0)
                                                : null;
                            ...
                            public StoreException handleException(final Exception e)
                                    throws StoreException {
                                throw new StoreException.ReadException(
                                        "Unable to get latest revision", e);
                            }
                        });
    }
                ...
                .doWithClientAndRepository(
                        new SvnPersisterCore.SvnOperation<List<Revision>>() {
                            ...
                            public List<Revision> execute(
                                    ...
                                    throws Exception {
                                return getSVNLogs(
                                        clientManager, targetPaths, SVNRevision.HEAD, start, limit);
                            }
                            ...
                            public StoreException handleException(final Exception e)
                                    throws StoreException {
                                throw new StoreException.ReadException(
                                        "Unable to get matrix history", e);
                            }
                        });
    }
    ...
    private List<Revision> getSVNLogs(
        ...
        try {
            ...
            // In order to get history is "descending" order, the startRevision should be the one
            // closer to HEAD
            logClient.doLog(
                    svnUrl,
                    paths, /* pegRevision */
    ...
    public void verifySetup() throws StoreException {
        final Long latestRevision =
                getSvnCore()
                        .doWithClientAndRepository(
                                new SvnPersisterCore.SvnOperation<Long>() {
                                    @Override
                                    public Long execute(
                                    ...
                                    public StoreException handleException(final Exception e)
                                        ...
                                        throw new StoreException(
                                                "Failed to get latest revision for svn-path: "
                                                        + svnUrl,
                                                e);
                                    }
                                });
        if (latestRevision <= 0) {
            throw new StoreException(
                    "Found non-positive revision (" + latestRevision + ") for svn-path: " + svnUrl);
        }
    }
