...
public class TestValueExtractors {
    ...
    public void testHeaderExtractor() throws Exception {
        ...
        Assert.assertNull("Request missing header value should return null", userAgentExtractor.extract(request));
    ...
    public void testChainedExtractor() throws Exception {
        ...
        final ValueExtractor chainedExtractor = ValueExtractors.chain(uaQueryParameter, userAgentHeader);
        final MockHttpServletRequest request = new MockHttpServletRequest();
        ...
        Assert.assertEquals("Query Parameter (listed first) should extract first and allow empty values", "", chainedExtractor.extract(request));
    }
}
