...
public abstract class AbstractGroupsPayload {
    ...
    private static final Function<Object, Long> LONG_CONVERTER =
            o -> convertToNumber(o).longValue();
    /** Used because ObjectMapper does not always read in doubles as doubles (can be integer) */
    private static final Function<Object, Double> DOUBLE_CONVERTER =
            o -> convertToNumber(o).doubleValue();
    /** Used only to provide better error messages for bad type, allows null (reason unknown) */
    public static final Function<Object, String> STRING_CONVERTER =
            o -> {
    ...
    @Nullable
    protected String convertToStringValue(final Payload payload, final String payloadMapKey)
    ...
    protected Long convertToLongValue(final Payload payload, final String payloadMapKey)
            throws IllegalArgumentException {
        return extractNonNullValueFromMapPayload(payload, payloadMapKey, LONG_CONVERTER);
    }

    protected Double convertToDoubleValue(final Payload payload, final String payloadMapKey)
            throws IllegalArgumentException {
        return extractNonNullValueFromMapPayload(payload, payloadMapKey, DOUBLE_CONVERTER);
    }

    protected String[] convertToStringArray(final Payload payload, final String payloadMapKey)
            throws IllegalArgumentException {
        // historically, null values are allowed in the result, reasons unknown
        return extractNonNullValueFromMapPayload(payload, payloadMapKey, o -> convertToList(o))
                .stream()
                .map(STRING_CONVERTER)
                .toArray(String[]::new);
    }

    /** Converts a list of Numbers in map payload to Array of Long if not possible */
    protected Long[] convertToLongArray(final Payload payload, final String payloadMapKey)
            throws IllegalArgumentException {
        return extractListWithouNullsFromMapPayload(payload, payloadMapKey, o -> convertToList(o))
                .stream()
                .map(LONG_CONVERTER)
                .toArray(Long[]::new);
    }
    ...
    /**
     * Converts a list of Numbers in map payload to Array of Double, throws RuntimeException if not
     * possible
     */
    protected Double[] convertToDoubleArray(final Payload payload, final String payloadMapKey)
            throws IllegalArgumentException {
        return extractListWithouNullsFromMapPayload(payload, payloadMapKey, o -> convertToList(o))
                .stream()
                .map(DOUBLE_CONVERTER)
                .toArray(Double[]::new);
    }
    ...
    private static <T> List<T> extractListWithouNullsFromMapPayload(
        ...
        if (list.contains(null)) {
            throw new NullPointerException(
                    "Null payload list value for constructor for key '"
                            + payloadMapKey
                            + '\''
                            + " in Payload: "
                            + payload);
     ...
     * @throws IllegalArgumentException when payload is null, payload.map is null, map does not
     *     contain key
    ...
    private static <T> T extractNonNullValueFromMapPayload(
        ...
        return extractValueFromMapPayload(payload, payloadMapKey, converter)
                .orElseThrow(
                        () ->
                                new NullPointerException(
                                        "Null payload value for constructor for key '"
                                                + payloadMapKey
                                                + '\''
                                                + " in Payload: "
                                                + payload));
    ...
    private static <T> Optional<T> extractValueFromMapPayload(
            ...
            final String payloadMapKey,
            final Function<Object, T> converter) {
        final Optional<Map<String, Object>> payloadMapOpt =
                Optional.ofNullable(payload).map(Payload::getMap);
        ...
        throw new IllegalArgumentException(
                "Missing payload for constructor for key '"
                        + payloadMapKey
                        + '\''
                        + " in Payload: "
                        + payload);
