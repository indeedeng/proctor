...
/**
 * A convenience class meant to provide easy functions for creating the various javascript payloads
 * proctor requires.
 */
public class ProctorJavascriptPayloadBuilder {
    ...
    public ProctorJavascriptPayloadBuilder(
            final AbstractGroups testGroups, final NameObfuscator testNameObfuscator) {
    ...
    public final <E extends Test> List<List<Object>> buildAlphabetizedListJavascriptConfig(
            final E... tests) {
        return testGroups.getJavaScriptConfig(tests);
    }

    /**
     * Generates a Map that be serialized to JSON and used with indeed.proctor.groups.init and
     * indeed.proctor.groups.inGroup(tstName, bucketValue) tstName is obfuscated by hashing the
     * tstName
     *
     * @param <E> Generic Type of Test
     * @param tests an alphabetical list of Test enums from your generated proctor java subclass of
     ...
     */
    public final <E extends Test> Map<String, List<Object>> buildObfuscatedJavaScriptConfigMap(
            final E... tests) {
        final ImmutableMap.Builder<String, List<Object>> result = ImmutableMap.builder();
        for (final Test test : tests) {
            final List<Object> testValueAndPayload =
                    Arrays.asList(
