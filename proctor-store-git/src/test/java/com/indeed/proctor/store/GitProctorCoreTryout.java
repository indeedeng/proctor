...
public class GitProctorCoreTryout {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (final FileBasedProctorStore store = new GitProctor(gitProctorCore, TEST_DEFINITION_DIRECTORY)) {
            ...
            LOGGER.debug("Testmatrix size: " + store.getCurrentTestMatrix().getTestMatrixDefinition().getTests().size());
            LOGGER.debug("Testmatrix size again: " + store.getCurrentTestMatrix().getTestMatrixDefinition().getTests().size());
        } finally {
