...
public abstract class FileBasedProctorStore implements ProctorStore {
    ...
    protected FileBasedProctorStore(
            final FileBasedPersisterCore core, final String testDefinitionsDirectory) {
        this.core = core;
    ...
    protected static <T> boolean writeIfChanged(final File f, final T newThing)
            throws StoreException.TestUpdateException {
        if (f.exists()) {
            try {
                final T currentThing =
                        (T) OBJECT_MAPPER.readValue(f, (Class<T>) newThing.getClass());
                if (currentThing.equals(newThing)) {
                    return false;
                }
            } catch (final IOException e) {
                throw new StoreException.TestUpdateException(
                        "Unable to parse instance of "
                                + newThing.getClass().getCanonicalName()
                                + " from "
                                + f,
                        e);
            }
    ...
    protected static <T> void writeThing(final File f, final T newThing)
            throws StoreException.TestUpdateException {
        try {
            OBJECT_MAPPER.writerWithDefaultPrettyPrinter().writeValue(f, newThing);
        } catch (final IOException e) {
            throw new StoreException.TestUpdateException(
                    "Unable to write instance of "
                            + newThing.getClass().getCanonicalName()
                            + " to "
                            + f,
                    e);
    ...
    public final TestMatrixVersion getTestMatrix(final String fetchRevision) throws StoreException {
        ...
        if (LOGGER.isDebugEnabled()) {
            ...
            LOGGER.debug(
                    String.format(
                            "Took %d ms to identify %d potential tests",
                            elapsed, result.getTests().size()));
        }
        if (result == null) {
        ...
        for (final TestVersionResult.Test testDefFile : result.getTests()) {
            ...
            final TestDefinition testDefinition =
                    getTestDefinition(testDefFile.getTestName(), testDefFile.getRevision());
            if (LOGGER.isTraceEnabled()) {
                final long elapsed = System.currentTimeMillis() - startForTest;
                LOGGER.trace(
                        String.format(
                                "Took %d ms to load %s (r%s) %s",
                                elapsed,
                                testDefFile.getTestName(),
                                testDefFile.getRevision(),
                                testDefinition == null ? "unsuccessfully" : "successfully"));
            }
            if (testDefinition == null) {
                LOGGER.info(
                        "Returning null TestMatrix because "
                                + testDefFile.getTestName()
                                + " returned null test-definition.");
                return null;
            }
        ...
        if (LOGGER.isDebugEnabled()) {
            ...
            LOGGER.debug(
                    String.format(
                            "Took %d ms to load all %d tests", elapsed, testDefinitions.size()));
        }
    ...
    @Override
    public TestDefinition getTestDefinition(final String testName, final String fetchRevision)
            throws StoreException {
        try {
            return getFileContents(
                    TestDefinition.class,
                    new String[] {
                        getTestDefinitionsDirectory(), testName, TEST_DEFINITION_FILENAME
                    },
                    null,
                    fetchRevision);
        } catch (final JsonProcessingException e) {
            throw new StoreException(
                    String.format("Unable to deserialize JSON for %s r%s", testName, fetchRevision),
                    e);
        }
    }
    ...
    private final <C> C getFileContents(
    ...
    protected final <T> boolean updateThing(
            final FileBasedProctorStore.RcsClient rcsClient, final File file, final T thing)
            throws Exception {
    ...
    public final void updateTestDefinition(
        ...
        LOGGER.info(
                String.format(
                        "Update Test Definition: username=%s author=%s testName=%s previousVersion=r%s",
                        changeMetadata.getUsername(),
                        changeMetadata.getAuthor(),
                        testName,
                        previousVersion));
        core.doInWorkingDirectory(
                changeMetadata,
                previousVersion,
                new ProctorUpdater() {
                    @Override
                    public boolean doInWorkingDirectory(
                            final RcsClient rcsClient, final File workingDir) throws Exception {
                        final File testDefinitionDirectory =
                                getTestDefinitionDirectoryForTest(testName, workingDir);
                        final File testDefinitionFile =
                                new File(
                                        testDefinitionDirectory
                                                + File.separator
                                                + TEST_DEFINITION_FILENAME);

                        if (!testDefinitionFile.exists()) {
                            throw new StoreException.TestUpdateException(
                                    "Attempting to update non-existent test " + testName);
                        }
                        //  this is easier than trying to get svnKit to do a useful diff
                        boolean thingsChanged =
                                updateThing(rcsClient, testDefinitionFile, testDefinition);
                        ...
                        thingsChanged =
                                updateThing(rcsClient, metaDataFile, metadata) || thingsChanged;
                        if (!thingsChanged) {
                            throw new StoreException.TestUpdateException(
    ...
    public final void addTestDefinition(
        ...
        LOGGER.info(
                String.format(
                        "Add Test Definition: %s %s", changeMetadata.getUsername(), testName));
        core.doInWorkingDirectory(
                changeMetadata,
                core.getAddTestRevision(),
                new ProctorUpdater() {
                    @Override
                    public boolean doInWorkingDirectory(
                            final RcsClient rcsClient, final File workingDir) throws Exception {
                        ...
                        if (testDefinitionFile.exists() || metaDataFile.exists()) {
                            throw new StoreException.TestUpdateException(
                                    "Supposedly new test '" + testName + "' already exists");
                        }
    ...
    public final void deleteTestDefinition(
        ...
        LOGGER.info(
                String.format(
                        "Delete Test Definition: %s %s r%s ",
                        changeMetadata.getUsername(), testName, previousVersion));
        core.doInWorkingDirectory(
                changeMetadata,
                previousVersion,
                new ProctorUpdater() {
                    ...
                    public boolean doInWorkingDirectory(
                        ...
                        if (!testDefinitionDirectory.exists()) {
                            throw new StoreException.TestUpdateException(
                                    "Unable to delete non-existent test " + testName);
                        }
    ...
    public interface ProctorUpdater {
        boolean doInWorkingDirectory(FileBasedProctorStore.RcsClient rcsClient, File workingDir)
                throws Exception;
    }
