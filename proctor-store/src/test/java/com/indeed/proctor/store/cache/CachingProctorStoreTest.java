...
public class CachingProctorStoreTest {
    ...
    private static final Thread.UncaughtExceptionHandler ASSERTION_FAILURE_HANDLER =
            (thread, throwable) -> {
    ...
    public void testAddTestDefinition() throws StoreException, InterruptedException {
        ...
        assertEquals(
                "description of tst3", testee.getCurrentTestDefinition("tst3").getDescription());
        assertEquals(
                "description of tst3",
                testee.getTestDefinition("tst3", testee.getLatestVersion()).getDescription());
    }
    ...
    public void testUpdateTestDefinition() throws StoreException, InterruptedException {
                        ...
                        () -> {
                            try {
                                final String lastRevision =
                                        testee.getHistory("tst1", 0, 1).get(0).getRevision();
        ...
        assertEquals(
                "description of tst1", testee.getCurrentTestDefinition("tst1").getDescription());
        assertEquals(1, testee.getHistory("tst1", 0, 2).size());
        startThreadsAndSleep(thread);
        assertEquals(
                "updated description tst1",
                testee.getCurrentTestDefinition("tst1").getDescription());
        assertEquals(2, testee.getHistory("tst1", 0, 2).size());
        assertEquals(
                "updated description tst1",
                testee.getTestDefinition("tst1", delegate.getLatestVersion()).getDescription());
        assertEquals(
                "description of tst1",
                testee.getTestDefinition("tst1", initialRevision).getDescription());
    }

    @Test
    public void testUpdateTestDefinitionWithIncorrectPrevVersion()
    ...
    public void testTwoUpdates() throws StoreException, InterruptedException, ExecutionException {
        ...
        final String lastRevision = testee.getHistory("tst1", 0, 1).get(0).getRevision();
        final FutureTask<Boolean> future1 =
                getFutureTaskUpdateTestDefinition(
                        lastRevision, "tst1", newTst1, "update description tst1");
        final FutureTask<Boolean> future2 =
                getFutureTaskUpdateTestDefinition(
                        lastRevision, "tst1", newTst2, "update description tst1");
        ...
        assertEquals(
                thread1UpdateSuccess ? description1 : description2,
                testee.getCurrentTestDefinition("tst1").getDescription());
        assertFalse(testee.getRefreshTaskFuture().isCancelled());
    }

    private FutureTask<Boolean> getFutureTaskUpdateTestDefinition(
    ...
    @Test
    public void testOneUpdateAndOneAdd()
            throws StoreException, InterruptedException, ExecutionException {
        final TestDefinition newTst1 = createDummyTestDefinition("3", "tst1");
        newTst1.setDescription("updated description tst1");
        final TestDefinition tst4 = createDummyTestDefinition("4", "tst4");

        final FutureTask<Boolean> updateFuture =
                getFutureTaskUpdateTestDefinition(
                        "2", "tst1", newTst1, "Update description of tst1");
        final Thread addThread =
                new Thread(
                        () -> {
                            try {
                                testee.addTestDefinition(
        ...
        assertEquals(
                "description of tst4", testee.getCurrentTestDefinition("tst4").getDescription());
        if (updateSuccess) {
            assertEquals(
                    "updated description tst1",
                    testee.getCurrentTestDefinition("tst1").getDescription());
            ...
            assertEquals(
                    "Update description of tst1",
                    testee.getHistory("tst1", 0, 4).get(0).getMessage());
        }
    }
    ...
    public void testDelete() throws StoreException, InterruptedException {
        final String initialRevision = delegate.getLatestVersion();
        final TestDefinition tst1 =
                createDummyTestDefinition("3", "tst1"); /* this would not be actually used */
